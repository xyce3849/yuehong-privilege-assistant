...
internal object DeviceRequestProofFactory {
    ...
    fun compatibilitySubject(profile: DeviceProfile): String =
        MessageDigest.getInstance("SHA-256")
            .digest("${profile.modelName}\n${profile.systemVersion}".toByteArray(StandardCharsets.UTF_8))
...
internal object SignedServerResponse {
    fun verify(body: String, proof: DeviceRequestProof): JSONObject {
        val envelope = JSONObject(body)
        if (envelope.optInt("protocol") != 2 ||
            envelope.optString("key_id") != BuildConfig.SERVER_PROTOCOL_V2_KEY_ID.trim()
        ) {
        ...
        val publicKey = decodeBase64Url(BuildConfig.SERVER_PROTOCOL_V2_PUBLIC_KEY.trim())
